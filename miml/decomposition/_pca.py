...
from mipylib.numeric.special import gammaln

from ._base import _BasePCA
from ..utils.extmath import svd_flip, stable_cumsum, fast_logdet
...
class PCA(_BasePCA):
    ...
    def _fit_full(self, X, n_components):
        ...
        if n_components < min(n_features, n_samples):
            nv = explained_variance_[n_components:]
            if isinstance(nv, numbers.Number):
                self.noise_variance_ = nv
            else:
    ...
    def _more_tags(self):
        return {'preserves_dtype': [np.dtype.float64, np.dtype.float32]}